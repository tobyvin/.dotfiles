...
local function fetch_sponsor_segments(youtube_id)
	local args =
	...
	if sponsors and sponsors.stdout then
...
local function get_ranges_from_chapters()
		...
		if categories_string then
			...
			for category_name in string.gmatch(categories_string, "([^,]+),?%s?") do
				...
				if skipped_categories[category] then
					...
					table.insert(ch_ranges, { ["segment"] = { chapter.time, to }, ["category"] = category })
				end
			end
		end
	end
